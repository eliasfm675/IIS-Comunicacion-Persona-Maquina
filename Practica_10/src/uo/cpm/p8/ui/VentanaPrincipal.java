...
public class VentanaPrincipal extends JFrame {
	...
	private void cargaAyuda(){
		   ...
		   hb.enableHelpOnButton(getMntmContent(), "introduccion", hs);
		   hb.enableHelp(getListLibrary(), "anadir", hs);
		   hb.enableHelp(getListPlayList(), "reproducir", hs);
		   hb.enableHelp(getSliderVolumen(), "volumen", hs); //por defecto el foco está puesto aquí, ergo no se abriría la ayuda "general"
		 }
	...
	public VentanaPrincipal(MusicPlayer mp) {
		addWindowListener(new WindowAdapter() {
			...
			public void windowOpened(WindowEvent e) {
				getMnFile().requestFocus(); // se ha de poner esto para que no pase lo de que te salga el html del slider en vez del general
			}
		});
	...
	private JFileChooser getSelector() {
		if(selector==null) {
			...
			//selector.setCurrentDirectory(new File(System.getProperty("user.dir"))); // fija el directorio de ejecuci�n del programa
	...
	private JMenuItem getMntmContent() {
		if (mntmContent == null) {
			mntmContent = new JMenuItem("Help content");
			mntmContent.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_H, InputEvent.CTRL_DOWN_MASK));
		}
		return mntmContent;
	}
}
