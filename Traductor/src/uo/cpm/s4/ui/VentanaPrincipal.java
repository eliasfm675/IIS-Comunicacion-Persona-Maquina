...
public class VentanaPrincipal extends JFrame {
	...
	private ProcessFocus pf = new ProcessFocus();
	private ProcessWindow pw = new ProcessWindow();
	private ProcessMouse pm = new ProcessMouse();
	private ProcessAction pa = new ProcessAction();
	private JCheckBox chckbxDigits;
	private ProcessKey pk = new ProcessKey();
	private ProcessCloseUp pc = new ProcessCloseUp();
	...
	private class ProcessWindow extends WindowAdapter{
		@Override
	...
	private class ProcessMouse extends MouseAdapter{
		@Override
	...
	private class ProcessAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			String input = JOptionPane.showInputDialog(null, "Seleccione un nombre para guardar el documento", "");
			getEtDocOriginal().setText(input);
		}
		
	}
	private class ProcessKey extends KeyAdapter{

		@Override
		public void keyTyped(KeyEvent e) {
			char pressedKey = e.getKeyChar();
			if(Character.isDigit(pressedKey) && !getChckbxDigits().isSelected()) {
				e.consume();
			}
			
		}
	}
	private class ProcessCloseUp implements ActionListener{
	...
	private void confirm() {
		if(JOptionPane.showConfirmDialog(null, "¿Deseas guardar los cambios?")==JOptionPane.YES_OPTION) {
	        System.exit(0);  // sin implementar la persistencia de datos
		}else {
			System.exit(0);
		}
	}
	...
	private JToolBar getBarraHerramientas() {
		if (barraHerramientas == null) {
			...
			barraHerramientas.add(getChckbxDigits());
		}
	...
	private JButton getBtGuardar() {
		if (btGuardar == null) {
			...
			btGuardar.addMouseListener(pm);
			btGuardar.addActionListener(pa);
		}
		return btGuardar;
	}
	...
	private JButton getBtImprimir() {
		if (btImprimir == null) {
			...
			btImprimir.addMouseListener(pm);
		}
		return btImprimir;
	}
	...
	private JTextArea getArOriginal() {
		if (arOriginal == null) {
			...
			arOriginal.addFocusListener(pf);
			arOriginal.setLineWrap(true);
			arOriginal.addKeyListener(pk);
		}
		return arOriginal;
	}
	...
	private JTextArea getArTraducido() {
		if (arTraducido == null) {
			...
			arTraducido.addFocusListener(pf);
	...
	private JCheckBox getChckbxDigits() {
		if (chckbxDigits == null) {
			chckbxDigits = new JCheckBox("\u00BFInclu\u00EDr n\u00FAmeros en la traducci\u00F3n?");
			chckbxDigits.setSelected(true);
		}
		return chckbxDigits;
	}
} 
