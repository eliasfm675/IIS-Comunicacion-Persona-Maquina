...
//			getTextAreaPedidos().setText(pz.getOrderToStringShoppingCart());
	...
	private void cargaAyuda(){
		   ...
		   hb.enableHelp(getPanelPago(), "pago", hs); //por defecto el foco está puesto aquí, ergo no se abriría la ayuda "general"
		 }
	...
	protected void confirmChoice() {
		if(JOptionPane.showConfirmDialog(null, "¿Estás seguro/a de que quieres salir de la aplicación?")==JOptionPane.YES_OPTION) {
			System.exit(0);
		}
	}
	...
	protected void cambiarPanelAMenu(ActionEvent e) {
		...
		if(String.valueOf(getPasswordFieldReserva().getPassword()).isBlank() || String.valueOf(getPasswordFieldReserva().getPassword()).isEmpty() || getTextFieldDNI().getText().isBlank() || getTextFieldDNI().getText().isEmpty() || !pz.isUserAClient(getTextFieldDNI().getText(), String.valueOf(getPasswordFieldReserva().getPassword()))) {
			JOptionPane.showMessageDialog(null, "Los datos introducidos no son válidos", "Uniovi's Error", JOptionPane.WARNING_MESSAGE);
		}else {
			changeFrame(e);
...
//			pz.getClient().editPlayState(true);
			...
			JOptionPane.showMessageDialog(null, "¡Pedido recibido en cocina!", "Recepción del pedido", JOptionPane.INFORMATION_MESSAGE);
			Juego jg = new Juego();
			...
			JOptionPane.showMessageDialog(null, "Lo sentimos, sólo se puede jugar una vez por reserva", "Error", JOptionPane.WARNING_MESSAGE);
		}else {
			JOptionPane.showMessageDialog(null, "¡Pedido recibido en cocina!", "Recepción del pedido", JOptionPane.INFORMATION_MESSAGE);
			pz.getClient().editTimesOrdered(pz.getClient().getTimesOrdered()+1);
			pz.saveOrder();
	...
	protected void generarBotonesMenu(int filter) {
		...
		for(int i=0; i<menuModel.length; i++) {
			...
			if(i==0 && menuModel[i].getType().equals("Pizza")) {
				...
				j.setText("¡Pizza del día! - " + j.getText());
			}
