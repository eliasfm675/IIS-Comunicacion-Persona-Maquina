...
//	}
	...
	private void compruebaFin() {
		if(spi.isFinJuego() && spi.isInvaderShot()) {
			habilitaTablero(false);
			for(int i=0; i<getGridPanel().getComponentCount(); i++) {
				pintaCasilla(i);
			}
			JOptionPane.showMessageDialog(this, "Partida finalizada - ¡Invasor Eliminado!", "Space Invaders EII", JOptionPane.INFORMATION_MESSAGE);
		}else if(spi.isFinJuego() && (!spi.isInvaderShot()) && (!spi.isMeteoriteShot())) {
			habilitaTablero(false);
			for(int i=0; i<getGridPanel().getComponentCount(); i++) {
				pintaCasilla(i);
			}
			JOptionPane.showMessageDialog(this, "Partida finalizada - ¡Te quedaste sin disparos!", "Space Invaders EII", JOptionPane.INFORMATION_MESSAGE);
		}else if(spi.isFinJuego() && spi.isMeteoriteShot()) {
			habilitaTablero(false);
			for(int i=0; i<getGridPanel().getComponentCount(); i++) {
				pintaCasilla(i);
			}
			getTextFieldScore().setText(String.valueOf(0));
			JOptionPane.showMessageDialog(this, "Partida finalizada - ¡Meteorito Disparado!", "Space Invaders EII", JOptionPane.INFORMATION_MESSAGE);
		}
	...
	private void despintaCasilla(int i) {
		ImageIcon imagen = new ImageIcon(VentanaPrincipal.class.getResource(""));
		((JButton)getGridPanel().getComponent(i)).setIcon(imagen);
		((JButton)getGridPanel().getComponent(i)).setDisabledIcon(imagen);
		validate();
	}
